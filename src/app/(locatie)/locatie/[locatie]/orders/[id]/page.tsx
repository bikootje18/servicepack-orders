    ...
    <div className="max-w-3xl">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 mb-4 shadow-sm">
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div className="text-center">
            ...
            <p className="text-2xl font-bold text-gray-900 tabular-nums leading-none">{formatAantal(order.order_grootte)}</p>
          </div>
          <div className="text-center border-x border-gray-100">
            <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-gray-400 mb-1">Gereed</p>
            <p className="text-2xl font-bold text-emerald-600 tabular-nums leading-none">{formatAantal(totaalGeleverd)}</p>
          </div>
          <div className="text-center">
            <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-gray-400 mb-1">Resterend</p>
            <p className={`text-2xl font-bold tabular-nums leading-none ${resterend === 0 ? 'text-gray-300' : 'text-amber-600'}`}>
      ...
      {kanGereedMelden && (
        <form
          action={async () => {
            'use server'
            await meldOrderGereed(id, locatie)
          }}
          className="mb-6"
        >
          <button
            type="submit"
            className="w-full py-3 px-6 rounded-xl text-sm font-semibold bg-green-50 text-green-700 border border-green-200 hover:bg-green-100 transition-colors"
          >
            Gereed melden
          </button>
        </form>
