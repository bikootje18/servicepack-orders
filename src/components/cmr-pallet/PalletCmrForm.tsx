        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 text-xs text-gray-500 font-medium">
              ...
              <th className="text-left py-2">Pallets × Kratten/pallet</th>
            </tr>
          </thead>
          <tbody>
            {PALLET_PRODUCTEN.map((product, i) => {
              ...
              return (
                <tr
                  key={i}
                  className={`border-b border-gray-100 align-top ${staat.geselecteerd ? 'bg-blue-50' : ''}`}
                >
                  <td className="py-2">
                    <input
                      type="checkbox"
                        ...
                        {staat.entries.map((entry, ei) => (
                          <div key={ei} className="flex items-center gap-1">
                            ...
                            <input
                              ...
                              className="form-input w-20 tabular-nums"
                            />
                            <span className="text-xs text-gray-400">kratten</span>
                            {ei === staat.entries.length - 1 && (
                              <button
                                type="button"
                                onClick={() => addVasteEntry(i)}
                                className="text-blue-500 hover:text-blue-700 text-xs px-1 font-bold"
                                title="Extra entry toevoegen"
                              >
                                +
                              </button>
                            )}
